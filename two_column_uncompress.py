...
if __name__ == '__main__':
    ...
    for filename in file_names:
        ...
        # 读取Template，为list
        templates1 = pd.read_csv(f'template_file/{filename}.csv',header=None,dtype=str)
        # 读取Template，为list
        templates2 = pd.read_csv(f'template_file/difference-{filename}.csv',header=None,dtype=str)
        
        uncompress_xz_to_csv(f'compress_file/difference-difference-{filename}.csv.xz', f'compress_file/difference-difference-{filename}.csv')
        # 读取压缩后数据，按照从左至右遍历解压
        bitmaps = pd.read_csv(f'compress_file/bitmap-difference-{filename}.csv',header=None,dtype=str)

        differences = pd.read_csv(f'compress_file/difference-difference-{filename}.csv',header=None,dtype=str)
        ...
        for i in range(0, row_number):
            for j in range(0, col_number):
                temp_i = i // 50
                temp_j = j
                template1 = str(templates1.iloc[temp_i, temp_j])
                template2 = str(templates2.iloc[temp_i, temp_j])
                ...
                if hex_string == 'nan':
                    ...
                    for bin_idx in range(0, len(binary_string), 2):
                        if binary_string[bin_idx:bin_idx+2] == '00':
                            ...
                            bitmap = bitmap + ' '
                        elif binary_string[bin_idx:bin_idx+2] == '11':
                            bitmap = bitmap + '2'
                ...
                for temp in template1:
                    ...
                    if temp == ',':
                        ...
                        elif bitmap[bit_idx] == '2':
                            uncompressed_data += template2[temp_idx]
