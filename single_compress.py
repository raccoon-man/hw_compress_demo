    ...
    for filename in file_names:
        ...
        for i in range(0, len(df.columns), len(df.columns)):
            ...
            for block in block_list:
                ...
                start_time = time.time()
                dt, diff = compress.compress_block(template_final, block)
                end_time = time.time()
        ...
        df.astype(str).to_csv(f'compress_file/single-bitmap-{filename}.csv',index=False,header=False)
        compress_csv_with_xz(f'compress_file/single-bitmap-{filename}.csv', 6)
        ...
        df.astype(str).to_csv(f'compress_file/single-difference-{filename}.csv',index=False,header=False)
        compress_csv_with_xz(f'compress_file/single-difference-{filename}.csv',6)
        ...
        df.astype(str).to_csv(f'template_file/single-{filename}.csv',index=False,header=False)
