import pandas as pd
import os
from segmentnumber import get_min_max_signed, is_within_32bit_range
import numpy as np
...
# 分号列信息，用于存储到metadata文件
separator_columns_info = []
        ...
        for column_name in column_data_dict.keys():
            if column_name in column_data_dict:
                ...
                if column_name != 'gridid':
                    ...
                    # 进入切分前的数值范围判断（使用无符号与有符号两种方式择优）
                    column_data = column_data.fillna('').astype(str)

                    min_val, max_val, has_negative = get_min_max_signed(column_data)
                    signed = has_negative
                    within_32bit = is_within_32bit_range(min_val, max_val, signed)

                    if not within_32bit:
                        with timer("pretreatment", "segment"):
                            dfn = segmentnumber.SegmentExecute(column_data, column_name)
...
def extract_process_to_dict(column_data, column):
    ...
    with timer("pretreatment", "extract"):
        ...
        for i in range(df_values.shape[1]):
            ...
            # 创建带有正确名称的 Series
            result_dict[f'{column}_value_{i+1}'] = pd.Series(new_values, name=f'{column}_value_{i+1}')
            
            # 处理分号列，注意分号列比数值列少一列，且跳过最后一个分号
            if i < df_separators.shape[1] - 1:
                # 将分号列信息添加到全局变量中，格式为 h_x_s_y:;
                separator_col_name = f'{column}_separator_{i+1}'
                short_name = separator_col_name.replace('head_', 'h').replace('body_', 'b').replace('_separator_', '_s_').replace('_value_', '_v_')
                separator_columns_info.append(f"{short_name}:;")
                # 不再将分号列添加到 result_dict 中
    return result_dict
...
def save_custom_txt_from_json(total_json: dict, file_name: str):
    """
    将 total_json 中的 dict、single、no 和 separator 部分格式化后写入 txt 文件。
    """
    with timer("file_write"):
        ...
        with open(output_path, 'w', encoding='utf-8') as file:
            ...
            # 写入 separator 部分（分号列信息）
            for sep_col in total_json.get('separator', []):
                file.write(f'{sep_col}\n')
...
if __name__ == "__main__":
    ...
    with timer("total"):
        ...
        for file_name in csv_files:
            ...
            # 将分号列信息添加到 total_json
            total_json['separator'] = separator_columns_info
